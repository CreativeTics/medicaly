...
const filter = (items, listFilter) => {
  listFilter = listFilter.map((_) => _[props.valueKey])
  items = items.filter((item) => listFilter.indexOf(item[props.valueKey]) == -1)
  return items
}
...
const openSelect = () => {
  ...
  if (props.modelValue.length == props.options.length) {
    ...
    let itemSelected = itemsList.value.filter(
      (item) => props.modelValue.indexOf(String(item[props.valueKey])) > -1
    )
    ...
    <div
      ...
      <div class="card-container bg-white rounded-lg overflow-hidden">
        <div>
          ...
          <article>
            <div class="p-3">
              <d-text-field placeholder="Buscar..." v-model="searchValue"></d-text-field>
            </div>
            ...
            <div class="flex flex-row items-start">
              <div
                ...
                <d-list
                  v-for="(item, index) in itemsList.filter(function (x) {
                    return (
                      searchValue == '' ||
                      x[showKey].toUpperCase().indexOf(searchValue.toUpperCase()) > -1
                    )
                  })"
                  :key="index"
