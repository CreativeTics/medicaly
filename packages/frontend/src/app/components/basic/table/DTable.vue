...
const emitSelected = (selected: any) => emit("selected", selected);
const emitPerpage = (perPage: any) => emit("perPage", perPage);
const emitCalcOffset = (offset: any) => emit("offset", offset);
const emitOrderSort = (sort: any, columnName: any) =>
  emit("sort", [sort, columnName]);
...
const sort = (name: any, index: number) => {
  if (name != undefined) {
    ...
    >
      <table class="min-w-full table-auto shadow-lg">
        ...
        <thead class="h-14 sticky top-0">
          <tr class="bg-white">
            <th v-if="selectOptions" class="p-1 bg-white" id="">
              <input
                type="checkbox"
                v-model="selectAll"
                @click="selectAllRow"
          ...
          >
            ...
            <td class="px-3" v-for="item in columns" v-bind:key="item">
              ...
              >
                <template
                  v-if="
                    item.name.indexOf('updated') >= 0 ||
                    item.name.indexOf('created') >= 0
                  "
                >
        ...
        <tfoot class="h-16 bg-white sticky bottom-0">
          <tr>
            <th :colspan="columns.length + 1" id="foot">
              <DPagination
                v-if="perPageDropdown?.length > 1"
                :per-page="perPage"
