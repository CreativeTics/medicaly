  ...
  <div>
    <div class="w-full min-w-full" v-if="mode == 'edit'">
      ...
      <label
        ...
        <span v-if="modelValue" class="flex flex-col justify-center items-center">
            ...
            modelValue.indexOf('\\') >= 0
              ? modelValue
                ...
                ? modelValue
                    ...
                    modelValue
                      .split('\\')
                      [modelValue.split('\\').length - 1].split('UpHere').length - 1
                  ]
                : modelValue.split('\\')[modelValue.split('\\').length - 1]
              : modelValue
                  .split('/')
                  [modelValue.split('/').length - 1].indexOf('UpHere') >= 0
              ? modelValue.split('/')[modelValue.split('/').length - 1].split('UpHere')[
                  modelValue.split('/')[modelValue.split('/').length - 1].split('UpHere')
                    .length - 1
                ]
              : modelValue.split('/')[modelValue.split('/').length - 1]
    ...
    <div v-else>
      ...
      <div class="flex mt-1">
        <input
          ...
          :class="[{ 'bg-gray-200': disabled, 'pointer-events-none': disabled }]"
          type="text"
          :value="modelValue"
          :disabled="mode == 'view'" />
        <div>
