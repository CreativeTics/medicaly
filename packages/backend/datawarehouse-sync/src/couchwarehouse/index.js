...
const spoolChanges = async (opts, theSchema, maxChange) => {
  ...
  if (opts.verbose) {
    bar = new ProgressBar(
      "downloading " + opts.database + " [:bar] :percent :etas",
      { total: maxChange, width: 30 }
    );
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    func
      .apply(changesReader, [params])
      .on("batch", async (b, done) => {
        if (b.length > 0) {
...
const monitorChanges = async function (opts, theSchema, lastSeq) {
  ...
  return new Promise((resolve, reject) => {
    ...
    cr.start({ since: lastSeq, includeDocs: true })
      .on("batch", async (b, done) => {
        if (opts.verbose) {
