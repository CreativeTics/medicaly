...
const initialise = async (reset) => {
  ...
  await client.query(
    "CREATE TABLE IF NOT EXISTS couchwarehouse_checkpoints (id SERIAL PRIMARY KEY, tablename TEXT, seq TEXT)"
  );
...
const writeCheckpoint = async (tablename, seq) => {
  const sql =
    "INSERT INTO couchwarehouse_checkpoints (tablename,seq) VALUES ($1,$2)";
  await client.query(sql, [tablename, seq]);
};

const getCheckpoint = async (tablename) => {
  const sql =
    "SELECT seq FROM couchwarehouse_checkpoints WHERE tablename=$1 ORDER BY id DESC LIMIT 1";
  let retval = null;
...
const message = (opts) => {
  console.log(
    "Run the following command to query your data warehouse in PostgreSQL:"
  );
  console.log("\n  $ psql\n");
